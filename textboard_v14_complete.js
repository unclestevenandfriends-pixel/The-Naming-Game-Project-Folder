...
const HistorySystem = {
    ...
    undo() {
        ...
        if (!editor || this.undoStack.length < 2) {
            Debug.log('⚠️ Nothing to undo', 'warn');
            return;
        }
        ...
        editor.focus();
        Debug.log('↩️ Undo applied', 'success');
    },
    ...
    redo() {
        ...
        if (!editor || this.redoStack.length == 0) {
            Debug.log('⚠️ Nothing to redo', 'warn');
            return;
        }
        this.isRestoring = true;
        ...
        this.isRestoring = false;
        editor.focus();
        Debug.log('↪️ Redo applied', 'success');
    }
};
...
const DrawingTool = {
    ...
    init() {
        ...
        if (!this.canvas) {
            Debug.log('Drawing canvas not found!', 'error');
            return;
        }
        ...
        Debug.log('Drawing tool initialized');
    },
    ...
    toggle() {
        ...
        Debug.log(`Drawing mode: ${this.isEnabled ? 'ON' : 'OFF'}`);
    },
...
const VoiceSystem = {
    ...
    init() {
        ...
        if (!SpeechRecognition) {
            Debug.log('⚠️ Speech Recognition not supported', 'warn');
            return;
        }
        ...
        this.recognition.onstart = () => {
            ...
            Debug.log('🎙️ Voice recording started', 'success');
        };
        ...
        this.recognition.onend = () => {
            ...
            if (this.pendingRestart) {
                ...
                try {
                    ...
                    Debug.log('❌ Voice restart error: ' + e.message, 'error');
                }
                return;
            }
        ...
        this.recognition.onerror = (e) => {
            Debug.log('❌ Voice error: ' + e.error, 'error');
            this.isRecording = false;
            this.updateButton(false);
        };
    ...
    toggle(editor, options = {}) {
        ...
        if (this.isRecording) {
            ...
            try {
                ...
                Debug.log('❌ Voice start error: ' + e.message, 'error');
            }
        }
    },
    ...
    handleVoiceInput(transcript) {
        ...
        if (cleanText && this.editor) {
            ...
            HistorySystem.saveState(true);
            Debug.log('📝 Voice: "' + cleanText.substring(0, 40) + '..."');
        }
...
const ImagePasteSystem = {
    ...
    init(editor) {
        ...
        Debug.log('🖼️ Image Paste system initialized');
    },
    ...
    handlePaste(e) {
        ...
        for (const item of items) {
            if (item.type.startsWith('image/')) {
                ...
                Debug.log('🖼️ Image pasted from clipboard', 'success');
            }
        }
    },
...
const ExportSystem = {
    async exportAsImage() {
        try {
            ...
            Debug.log('📸 Exported as image', 'success');
        } catch (err) {
    ...
    async exportAsDOCX() {
        try {
            ...
            Debug.log('📝 Exported as DOCX', 'success');
        } catch (err) {
...
const TextBoardSystem = {
    ...
    init() {
        ...
        if (!this.overlay || !this.editor) {
            Debug.log('TextBoard elements not found!', 'error');
            return;
        }
        ...
        VoiceSystem.init();
        ImagePasteSystem.init(this.editor);
        Debug.log('✅ TextBoard V14 initialized', 'success');
    },
    ...
    bindAllEvents() {
        ...
        document.getElementById('btn-mode-blank').addEventListener('click', () => this.applyMode('blank'));
        document.getElementById('btn-mode-convo').addEventListener('click', () => this.applyMode('conversation'));
        document.getElementById('btn-mode-dynamic').addEventListener('click', () => this.applyMode('dynamic'));
        ...
        document.getElementById('btn-bold').addEventListener('click', () => this.format('bold'));
        document.getElementById('btn-italic').addEventListener('click', () => this.format('italic'));
        document.getElementById('btn-underline').addEventListener('click', () => this.format('underline'));
        document.getElementById('btn-strike').addEventListener('click', () => this.format('strikethrough'));
        document.getElementById('btn-left').addEventListener('click', () => {
            if (ImagePasteSystem.selectedImg) ImagePasteSystem.alignImage('left');
            else this.format('justifyLeft');
        });
        document.getElementById('btn-center').addEventListener('click', () => {
            if (ImagePasteSystem.selectedImg) ImagePasteSystem.alignImage('center');
            else this.format('justifyCenter');
        });
        document.getElementById('btn-right').addEventListener('click', () => {
            if (ImagePasteSystem.selectedImg) ImagePasteSystem.alignImage('right');
            else this.format('justifyRight');
        });
        document.getElementById('btn-ul').addEventListener('click', () => this.format('insertUnorderedList'));
        document.getElementById('btn-ol').addEventListener('click', () => this.format('insertOrderedList'));
        document.getElementById('btn-indent').addEventListener('click', () => this.format('indent'));
        document.getElementById('btn-outdent').addEventListener('click', () => this.format('outdent'));
        ...
        document.getElementById('tb-mic-btn').addEventListener('click', () => VoiceSystem.toggle(this.editor, { autoSpag: false }));
        document.getElementById('tb-mic-spag-btn').addEventListener('click', () => VoiceSystem.toggle(this.editor, { autoSpag: true }));
        ...
        if (modeToggleBtn) {
            modeToggleBtn.addEventListener('click', () => {
                if (AI_CONFIG.modeToggle === 'REPLACE') {
                    AI_CONFIG.modeToggle = 'INSERT_BELOW';
                    modeToggleBtn.classList.add('active');
                    modeToggleBtn.title = 'Mode: Insert Below (click to toggle)';
                    this.showToast('Mode: Insert Below');
                } else {
                    AI_CONFIG.modeToggle = 'REPLACE';
        ...
        document.getElementById('btn-paraphrase').addEventListener('click', () => {
            if (AI_CONFIG.paraphraseStyle === 'Custom' && !AI_CONFIG.customStyleInstruction) {
                ...
                runAiTool('paraphrase', {
                    paraphraseStyle: AI_CONFIG.paraphraseStyle,
                    customStyleInstruction: AI_CONFIG.customStyleInstruction
                });
            }
        });
        ...
        document.getElementById('custom-style-cancel')?.addEventListener('click', () => {
            ...
            AI_CONFIG.paraphraseStyle = 'Formal';
        });
        ...
        document.getElementById('custom-style-apply')?.addEventListener('click', () => {
            ...
            AI_CONFIG.customStyleInstruction = customInput?.value || '';
            hideModal();
        ...
        const penBtn = document.getElementById('btn-pen');
        if (penBtn) penBtn.addEventListener('click', () => DrawingTool.toggle());

        document.querySelectorAll('.drawing-size-btn').forEach(btn => {
            btn.addEventListener('click', () => DrawingTool.setSize(btn.dataset.size));
        });

        document.querySelectorAll('.drawing-color-btn').forEach(btn => {
            btn.addEventListener('click', () => DrawingTool.setColor(btn.dataset.color));
        });

        const eraserBtn = document.getElementById('btn-eraser');
        if (eraserBtn) eraserBtn.addEventListener('click', () => DrawingTool.setEraser());

        const clearDrawingBtn = document.getElementById('btn-clear-drawing');
        if (clearDrawingBtn) clearDrawingBtn.addEventListener('click', () => DrawingTool.clear());
        ...
        this.editor.addEventListener('input', () => HistorySystem.scheduleSave());
        this.editor.addEventListener('blur', () => HistorySystem.saveState(true));
    ...
    open() {
        ...
        requestAnimationFrame(() => {
            ...
            setTimeout(() => DrawingTool.resizeCanvas(), 100);
        });
        this.isOpen = true;
        this.editor.focus();
        Debug.log('📋 TextBoard opened');
    },
    ...
    close() {
        ...
        if (VoiceSystem.isRecording && VoiceSystem.recognition) {
            VoiceSystem.recognition.stop();
        }

        Debug.log('📋 TextBoard closed');
    },
...
async function runAiTool(mode, options = {}) {
    ...
    try {
        ...
        const payload = {
            ...
            modeToggle: AI_CONFIG.modeToggle, // Read from config
